...
```yaml
global:
  baseInstance: ghostfolio   # release name used for `helm install <name> ./helm/base`
```
...
Values reference: [`helm/platform/values.yaml`](https://github.com/7KGroup/ghostfolio/blob/main/helm/platform/values.yaml)
...
Connection credentials are published to a `Secret` named `ghostfolio-app` that the base chart can pull via `envFrom`.
...
externalSecrets:
  ...
  data:
    - secretKey: DATABASE_URL
      remoteKey: ghostfolio/database
      property: url
    - secretKey: API_KEY
      remoteKey: ghostfolio/api
      property: key
```
...
The generated `Secret` is named after the application (`ghostfolio`). Reference it from the base chart's `envFrom`:
